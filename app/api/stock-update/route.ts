...
export async function POST(request: NextRequest) {
  try {
    const { productName, sku, operation, quantity, attributes } = await request.json()

    if (!productName || !sku || !operation || !quantity) {
      return NextResponse.json(
        { error: "Missing required fields: productName, sku, operation, quantity" },
        { status: 400 },
      )
    }
    ...
    if (isLocalhost) {
      ...
      return NextResponse.json({
        success: true,
        salesforceId: `demo-${Date.now()}`,
        message: `Stock ${operation === "in" ? "added" : "removed"} (demo mode - localhost detected)`,
        usingDemoMode: true,
    ...
    if (!instanceUrl || !clientId) {
      ...
      return NextResponse.json({
        success: true,
        salesforceId: `demo-${Date.now()}`,
        message: `Stock ${operation === "in" ? "added" : "removed"} (demo mode - Salesforce not configured)`,
        usingDemoMode: true,
      })
    }
    ...
    try {
      ...
      // Map attributes to Salesforce fields with correct field names
      const salesforceData: any = {
        Productname__c: String(attributes["Product name"] || productName || "Unknown Product"),
        Model__c: String(attributes["Model"] || sku || "Unknown Model"),
      }

      // Add optional fields with correct API names
      if (attributes["Phase"]) salesforceData.Phase__c = String(attributes["Phase"])
      if (attributes["Stage"]) salesforceData.Stage__c = Number(attributes["Stage"])
      if (attributes["Voltage"]) salesforceData.Voltage__c = String(attributes["Voltage"])
      if (attributes["Frequency"]) salesforceData.Frequency__c = String(attributes["Frequency"])
      if (attributes["Price"]) salesforceData.Price__c = Number(attributes["Price"])
      if (attributes["H.P."]) salesforceData.H_p__c = Number(attributes["H.P."])
      if (attributes["Category"]) salesforceData.Category__c = String(attributes["Category"])

      // Set quantity based on operation (positive for stock in, negative for stock out)
      salesforceData.Quantity__c = operation === "in" ? Number(quantity) : -Number(quantity)

      console.log("Creating Salesforce stock record with data:", JSON.stringify(salesforceData, null, 2))
      ...
      if (!createResponse.ok) {
        ...
        return NextResponse.json({
          success: true,
          salesforceId: `demo-${Date.now()}`,
          message: `Stock ${operation === "in" ? "added" : "removed"} (demo mode - Salesforce field error)`,
          usingDemoMode: true,
          salesforceError: `${createResponse.status}: ${errorMessage}`,
          errorDetails: errorDetails,
          note: "Some fields may not exist in your Salesforce object. Stock update saved to demo data instead.",
        })
      }
      ...
      try {
        ...
        return NextResponse.json({
          success: true,
          salesforceId: `demo-${Date.now()}`,
          message: `Stock ${operation === "in" ? "added" : "removed"} (demo mode - unexpected response format)`,
          usingDemoMode: true,
        })
      }
      ...
      return NextResponse.json({
        ...
        message: `Stock ${operation === "in" ? "added" : "removed"} and record created in Salesforce with ID: ${result.id}`,
        usingDemoMode: false,
      })
      ...
      let userMessage = `Stock ${operation === "in" ? "added" : "removed"} (demo mode)`

      if (salesforceError instanceof Error) {
        errorMessage = salesforceError.message

        if (errorMessage.includes("PUBLIC_URL_REQUIRED")) {
          userMessage = `Stock ${operation === "in" ? "added" : "removed"} (demo mode: Salesforce requires public URL)`
        } else if (errorMessage.includes("INVALID_CREDENTIALS")) {
          userMessage = `Stock ${operation === "in" ? "added" : "removed"} (demo mode: Invalid Salesforce credentials)`
        } else if (errorMessage.includes("INVALID_CLIENT")) {
          userMessage = `Stock ${operation === "in" ? "added" : "removed"} (demo mode: Invalid Salesforce app configuration)`
        } else {
          userMessage = `Stock ${operation === "in" ? "added" : "removed"} (demo mode: Salesforce connection failed)`
        }
      }
    ...
    return NextResponse.json({
      success: true,
      salesforceId: `demo-${Date.now()}`,
      message: "Stock updated (demo mode - API error)",
      usingDemoMode: true,
